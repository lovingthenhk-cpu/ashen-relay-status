...
目的: プレイテストから企画再編までのユーザー指示を、次のRで実際に処理する作業へ対応させる。状態: **現行の公開作業計画**（2026-09-25）。順序と受入は [完成工程](completion.html)、旧案の採否は [旧案の採用台帳](ideas.html)、全案の実装行は このページ、既存実装と検証は 内部の検証記録 を参照。物語の核心と隠し条件は本書に記さない。

## 1. 次に開始する作業

**R-27W1の静的監査とR-27W2の現世道路接合の機械検証は完了。R-27W3は実装中で未受入**。Lost Citiesの原生道路からAlex'sの小屋、梯子、Mowzie'sのフェラス室へ、3 seedで自然生成・実ブロック経路・原生boss・再起動を確認した。外観と人が実際に歩いた感覚は未確認。同じ街道にApotheosis原生塔への入口と原生lootを接合した。塔の4額縁は元Modと同じランダムGem抽選を使用し、再起動後は4個とも検出した。強制ロード直後の額縁表示と3 seed安定生成は未受入。

| 順 | 次の単位 | このチャットの要望へ届く具体的な成果 |
|---|---|---|
| 1 | **R-27W1（静的監査完了）** | 原生構造の候補と入口/寸法/権利、既存の独自建物の参照箇所、Lost Citiesの配置・空洞の原因を確定する。Dungeons Enhanced等の未導入候補も隔離試験の資料として載せる |
| 2 | **R-27W2（機械受入済み、目視未検証）** | 現世Lost Cities道路からAlex's小屋の梯子を通りMowzie's室へ降り、地上へ戻れる。Structure Void/地形上書き・通路未接続を新規3 seedで検証する |
...
R-27W2のコードと現世街区JSONは実装済み。W3は同じ街道の塔への分岐を作る。新規worldの検証には使い捨て環境を使い、プレイ中worldを断りなく削除しない。
...
| 他にも今遊べる場所を増やす。同じワールドから行ける導線を優先 | 施設が自然生成するだけで「遊べる」としない。試験用新規worldは生成検証に使い、プレイ用worldとは区別 | R-29A〜C/R-37以降。街の準備→原生遠征→固有drop→都市で更新→別攻略の往復を作る |

過去のOP付与、物資支給、TP、spectator/survival切替、ワールド削除・再起動、クライアント起動は、その時の試験環境を操作する個別指示だった。将来の長期worldを削除する恒久許可や、毎Rでクライアントを自動操作する許可にはしない。テスト時に必要なら 内部の運用手順 と 内部のテスト手順 に従い、Codexが専用server/selftest/ログで検証する。目視でしか判断できない景観・楽しさはユーザー本人が確認する意思を示しているが、Codex側の静的検査・専用server・selftestを先に完了し、目視未確認を合格にしない。Computer Useは禁止。
...
旧資料の全案を単一Rの巨大な実装にまとめない。X01〜X20は `R-40A〜D`、`R-41A〜E`、`R-42A〜F`、`R-43A〜E` にそれぞれ割り当てた。必要なModは1つずつ隔離し、固定jar/権利/stop対象/原生施設を調べた後に本番へ導入する。X15の二Modなど一単位に異なる体験が残る場合は分割する。R-44で全行を監査し、未達があれば次のRを作る。本筋と無関係に見える旧案も、三理由で除外できない限り、任意遠征・寄り道・装備選択のどこで意味を持つか試す。
