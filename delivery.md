...
**R-27W1の固定jar静的監査は完了。次の制作単位はR-27W2**。現世のLost Cities道路を実ブロックで調べ、Alex'sの小屋、梯子、Mowzie'sのフェラス室、帰路をつなぐ。新規3 seedと再起動で通路・衝突・元boss/dropを検査する。R-27W1では市街接続の実装も実world配置も行っていない。
...
R-27W1で確認した実IDと配置条件に基づいてW2のコード/JSONを確定する。新規worldの検証には使い捨て環境を使い、プレイ中worldを断りなく削除しない。
