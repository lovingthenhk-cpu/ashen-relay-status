...
目的: プレイテストから企画再編までのユーザー指示を、次のRで実際に処理する作業へ対応させる。状態: **現行の公開作業計画**（2026-09-24）。順序と受入は [完成工程](completion.html)、旧案の採否は [旧案の採用台帳](ideas.html)、全案の実装行は このページ、既存実装と検証は 内部の検証記録 を参照。物語の核心と隠し条件は本書に記さない。

## 1. 次に開始する作業

**R-27W1の静的監査とR-27W2の現世道路接合の機械検証は完了。次はR-27W3**。Lost Citiesの原生道路からAlex'sの小屋、梯子、Mowzie'sのフェラス室へ、3 seedで自然生成・実ブロック経路・原生boss・再起動を確認した。外観と人が実際に歩いた感覚は未確認。同じ街道にApotheosis塔への入口を作る。
